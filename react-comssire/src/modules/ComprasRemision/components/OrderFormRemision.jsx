export default function OrderFormRemision({
  readOnly = false,
  loading,
  folio,
  setFolio,
    ...
    <div className="compra-card p-5">
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-12 md:col-span-3">
          <label className="compra-label">Folio remisión</label>
          <input
        ...
        <div className="col-span-12 md:col-span-4">
          ...
          >
            ...
            {proveedores.map((p, idx) => {
              const value = String(p.clave ?? p.Clave ?? p.cveClpv ?? p.CveClpv ?? "");
              const label = String(
                p.nombre ?? p.Nombre ?? p.razonSocial ?? p.RazonSocial ?? p.clave ?? p.Clave ?? value
              );
              return (
                <option key={`${value || "prov"}-${idx}`} value={value}>
                  {label}
                </option>
              );
            })}
        ...
        <div className="col-span-12 md:col-span-3">
          ...
          >
            <option value="">Seleccionar...</option>
            {almacenes.map((a, idx) => {
              const value = String(a.cveAlm ?? a.CveAlm ?? a.numAlm ?? a.NumAlm ?? "");
              const label = String(a.descr ?? a.Descr ?? value);
              return (
                <option key={`${value || "alm"}-${idx}`} value={value}>
                  {label}
                </option>
              );
            })}
